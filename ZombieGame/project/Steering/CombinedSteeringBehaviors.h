...
public:
	...
	SteeringOutput CalculateSteering(float deltaT, const AgentInfo& aInfo) override;

	void SetBehaviorWeight(int index, float newWeight) { if (m_WeightedBehaviors.size() > index) m_WeightedBehaviors[index].weight = newWeight; }
